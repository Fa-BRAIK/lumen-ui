export const registerComponent = () => {
    ...
    const handleRoot = (el, Alpine, { min, max, step, minStepBetweenThumbs, defaultValue, rovingFocus, orientation, dir, disabled }) => {
        Alpine.bind(el, () => ({
            ...commonProps,
            'x-data'() {
                return {
                    ...
                    get trackElement() {
                        return this.$el.getAttribute('data-slot') === 'slider'
                            ...
                            : this.$el.closest('[data-slot="slider"]').querySelector('[data-slot="slider-track"]');
                    },

                    __getClosestThumbIndex(value, index = undefined) {
                        return this.__values.reduce((closestIndex, thumbValue, thumbIndex) => {
                            // Skip index if provided
                            if (index !== undefined && thumbIndex === index) return closestIndex;
                            if (closestIndex === undefined) return thumbIndex;

                            const closest = Math.abs(this.__values[thumbIndex] - value);
                            const diff = Math.abs(thumbValue - value);

                            return diff < Math.abs(closest - value) ? thumbIndex : closestIndex;
                        }, undefined) ?? -1;
                    },

                    __acceptsNewValue(newValue, index) {
                        const closestThumbIndex = this.__getClosestThumbIndex(newValue, index);
                        const closestThumbValue = this.__values[closestThumbIndex];

                        return closestThumbIndex === -1 
                    ...
                    __onValueChange(newValue, index) {
                        ...
                        if (newValue < this.min || newValue > this.max) {
                            console.warn(`Value ${newValue} is out of bounds for slider with min ${this.min} and max ${this.max}.`);
                            return;
                        }
                    ...
                    init() {
                        ...
                        if (thumbs.length !== this.__values.length) {
                            console.warn(`Number of thumbs (${thumbs.length}) does not match the number of values (${this.__values.length}).`);
                        }
                    }
                }
            },
    ...
    const handleTrack = (el, Alpine, {}) => {
        Alpine.bind(el, () => ({
            ...
            '@click'() {
                ...
                const mouseOnTrackPosition = this.orientation === 'horizontal' ? clientX : clientY
                const minPosition = this.orientation === 'horizontal' ? track.left : track.top
                const maxPosition = this.orientation === 'horizontal' ? track.right : track.bottom
                ...
                return {
                    get minRange() {
                        return this.minValue === this.maxValue
                            ? this.dir === 'ltr' ? this.min : this.minValue
                            : this.minValue;
                    },

                    get maxRange() {
                        return this.minValue === this.maxValue
                            ? this.dir === 'ltr' ? this.maxValue : this.max
                            : this.maxValue;
                    },
    ...
    const handleThumb = (el, Alpine, {}) => {
        Alpine.bind(el, () => ({
            ...
            '@keydown.left'() {
                if (this.disabled || ! this.rovingFocus || this.dir !== 'ltr' || this.orientation === 'vertical') {
                    return;
                }

                const newValue = Math.max(this.min, this.__value - this.step);
                const closestThumbValue = this.__values[this.__getClosestThumbIndex(this.__value, this.__index)];
            ...
            'x-data'() {
                return {
                    ...
                    __handleHorizontalDragging(event) {
                        ...
                        const clientX = event.clientX ?? event.touches[0].clientX;
                        const mouseOnTrackPosition = Math.min(Math.max(track.left, clientX), track.right);
                        const percentage = (mouseOnTrackPosition - minPosition) / (maxPosition - minPosition);
                        const value = Math.round(this.min + (percentage * (this.max - this.min)) / this.step) * this.step;
                    ...
                    init() {
                        ...
                        this.$watch('__value', newValue => {
                            ...
                            if (this.orientation === 'horizontal') {
                                this.$el.parentElement.style.setProperty('left', `${newValue}%`);
                        ...
                        this.$watch('__values', (newValues) => {
                            const newThumbValue = newValues[this.__index] ?? this.defaultValue[this.__index] ?? this.min;
                        ...
                        this.__index = Array.from(this.$el
                        ...
                        if (typeof this.__values[this.__index] === 'undefined') {
                            this.__values[this.__index] = this.defaultValue[this.__index] ?? this.min;
                        }
    ...
    Alpine.directive('slider', (el, {value, expression}, {Alpine, evaluate}) => {
